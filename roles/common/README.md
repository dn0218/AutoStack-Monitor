...
**基础包安装**: 安装 `podman`, `acl`, `sudo` 等必要组件 [cite: 109]。
**用户环境**: 创建部署用户 `deploy_user` 并配置无密码 sudo 权限 [cite: 110, 111]。
**持久化配置**: 通过 `loginctl enable-linger` 确保用户退出登录后容器继续运行 [cite: 112]。
**权限映射**: 配置 `/etc/subuid` 和 `/etc/subgid` 以支持 rootless 模式下的 UID 映射 [cite: 112]。
**API 激活**: 开启用户级别的 Podman Socket [cite: 112, 113]。
...
**Base Packages**: Installs `podman`, `acl`, `sudo`, and other essential components[cite: 109].
**User Setup**: Creates the `deploy_user` and configures passwordless sudo privileges[cite: 110, 111].
**Persistence**: Uses `loginctl enable-linger` to ensure containers remain running after the user logs out[cite: 112].
**Namespace Mapping**: Configures `/etc/subuid` and `/etc/subgid` to support UID mapping in rootless mode[cite: 112].
**API Activation**: Enables the user-level Podman Socket[cite: 112, 113].
